...
- Justify alignment: calculates per-line letter spacing as `(maxW - naturalW) / (charCount - 1)`. The last line of a multi-line block renders left-aligned (standard typographic justification). Single-line text with justify falls back to left-aligned.
