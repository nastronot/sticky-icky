...
- Text style toggles (All Caps, Small Caps, Italic) are non-destructive — the textarea value is never modified. Display text is derived: `allCaps || smallCaps → text.toUpperCase()`. Small caps renders originally-lowercase characters at 70% of the fitted font size. Italic prepends `'italic '` to the `ctx.font` string. All three toggles are independent and can combine.
